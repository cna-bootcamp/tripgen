...
본 문서는 AI 기반 여행 상세 일정 생성 서비스의 UI/UX 설계 내용을 체계적으로 정리한 화면 설계서입니다. 18개 유저스토리를 기반으로 설계된 12개 핵심 화면의 구조, 기능, 상호작용을 상세히 기술합니다.

### 설계 범위
- **총 화면 수**: 12개 핵심 화면 + 1개 인덱스 페이지
- **대상 서비스**: 5개 마이크로서비스 (PROF, ITGEN, LOC, MAP, REV)
- **유저스토리**: 18개 요구사항 반영 **(+1개 증가 - UI/UX 접근성 강화)**
...
- **🆕 주변장소검색 카드**: 대형 그라데이션 카드로 핵심 기능 강조
  ...
  - 기능 태그: 맛집, 숙소, 관광지, 카페
  - 호버 애니메이션 및 그림자 효과
- **통계 카드**: 클릭 가능한 "저장된 장소" 항목
- **프로젝트 카드**: 진행률, 썸네일, 기본 정보
- **추천 섹션**: 인기 여행지, 계절별 추천
- **플로팅 액션 버튼**: 우하단 2개 버튼 (여행계획, 장소검색)
...
### 🆕 표준화된 네비게이션 시스템
- **헤더 네비게이션**: 모든 화면에서 동일한 7개 메뉴 구조
  - 대시보드, 멤버관리, 일정설정, 이동수단, AI일정생성, 일정조회, 지도
  - 현재 페이지 active 상태 표시
  - 모바일에서 햄버거 메뉴로 전환
