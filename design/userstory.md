...
- 시나리오: 여행지별 일정 설정
  ...
  [자동 계산 및 검증]
  - 날짜 자동 계산: 이전 여행지 종료일이 다음 여행지 시작일
  - 숙박일 기반 종료일 자동 계산
  - 최소 1개 이상의 여행지 필수
  - 자동 저장 기능
...
- 시나리오: 장소 상세정보 조회
  ...
  - 최신 리뷰 5개 표시
    ...
    - 개별 별점
    - 리뷰 내용
  - 국내: "카카오 정책으로 리뷰는 구글 리뷰 제공" 안내
  - "더 많은 리뷰 보기" 링크 (구글맵 연결)
...
- 기술 태스크
  - 장소 정보 API 통합
    - 국내 장소
      - 카카오 로컬 API: 기본 정보, 평점, 주소, 카테고리
      - 구글 Places API: 리뷰만 조회 (최대 5개)
      - 리뷰 섹션에 "카카오 정책으로 리뷰는 구글 리뷰 제공" 안내 문구 표시
      - "더 많은 리뷰 보기"는 구글맵 링크로 연결
    - 해외 장소
      - 구글 Places API: 평점, 리뷰, 운영시간 등 전체 정보
      - 리뷰는 최대 5개까지만 표시
      - "더 많은 리뷰 보기"는 구글맵 링크로 연결
  - AI 추천 정보 연동
    - AI 서비스 API 호출
