...
public class RouteController {
    ...
    @GetMapping
    @Operation(summary = "경로 조회 (GET)", description = "출발지와 목적지 간 경로 정보를 조회합니다 (GET)")
    public ResponseEntity<RouteResponse> getRouteByParams(
            @Parameter(description = "출발지 위도", required = true) 
            @RequestParam Double fromLatitude,
            @Parameter(description = "출발지 경도", required = true) 
            @RequestParam Double fromLongitude,
            @Parameter(description = "도착지 위도", required = true) 
            @RequestParam Double toLatitude,
            @Parameter(description = "도착지 경도", required = true) 
